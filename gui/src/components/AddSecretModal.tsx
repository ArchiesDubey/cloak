      ...
      <div className="relative w-full max-w-md bg-surface rounded-xl border border-border-track shadow-modal overflow-hidden animate-fade-in">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border-subtle">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg bg-surface-active flex items-center justify-center border border-border-subtle text-white">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          {errorMessage && (
            <div className="px-3 py-2 rounded-lg bg-[#FF3F00]/10 border border-burnrate-critical/40 text-burnrate-critical text-xs font-mono">
              ⚠️ {errorMessage}
            </div>
          )}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={`flex items-center justify-center gap-2 p-2 rounded-lg text-xs font-medium border transition-all duration-150 ease-spring active:scale-[0.98] cursor-pointer ${
                  scope === 'global'
                    ? 'bg-surface-active text-white border-border-track shadow-sm'
                    : 'bg-surface text-[#808080] hover:text-white border-border-subtle hover:bg-surface-hover'
                }`}
              >
          ...
          {scope === 'project' && (
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-surface-active text-white font-mono text-xs rounded-lg border border-border-subtle focus:border-[#00FF88]/50 focus:ring-1 focus:ring-[#00FF88]/20 focus:outline-none"
              />
            </div>
          )}
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="text-xs font-medium text-[#808080]">
                Key Identifier
              </label>
              <span className="text-[10px] text-[#6E6E73] font-mono">ENV_VAR_NAME</span>
            </div>
            <input
              ref={keyInputRef}
              type="text"
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-[10px] text-[#6E6E73]">Hardware Encrypted</span>
            </div>
            <div className="relative flex items-center">
              <input
                ...
                placeholder="sk-proj-..."
                className="w-full pl-3 pr-10 py-2 bg-surface-active text-white font-mono text-xs rounded-lg border border-border-subtle focus:border-[#00FF88]/50 focus:ring-1 focus:ring-[#00FF88]/20 focus:outline-none"
              />
              <button
                type="button"
                onClick={() => setShowValue(!showValue)}
                className="absolute right-2 p-1 text-[#808080] hover:text-white cursor-pointer"
          ...
          <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-border-subtle">
            <button
              type="button"
              onClick={onClose}
              className="px-3.5 py-1.5 rounded-lg text-xs font-medium text-[#808080] hover:text-white hover:bg-surface-hover transition-colors cursor-pointer"
            >
              Cancel
            </button>
            <button
              ...
              className="px-4 py-1.5 rounded-lg text-xs font-semibold bg-white hover:bg-zinc-200 text-black transition-all duration-150 ease-spring active:scale-[0.98] cursor-pointer disabled:opacity-50"
            >
